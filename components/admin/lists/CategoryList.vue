  ...
  <tr v-if="!deleted">
    <td class="pa-0">
      <v-list-item class="pa-0 my-1">
        <v-avatar class="mr-3">
          <img
            :src="item.image_thumb ? $mediaBase + item.image_thumb : '/img/placeholder.jpg' "
          >
        </v-avatar>
        <h3> {{ $lang(item.name) }} </h3>
      </v-list-item>
    </td>
  ...
  methods: {
    deleteItem (id) {
      if (confirm(this.$multiLang('AreYouSure'))) {
        this.loading = true
        this.$api.post('admin/deletecategory/' + id).then((res) => {
          this.loading = false
          if (res.data.status) {
    ...
    changeStatus () {
      ...
      this.$api.post('admin/category-status/' + this.item.id, formData)
    }
  }
}
</script>
