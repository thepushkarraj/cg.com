<template>
  <one-row class="mb-5">
    ...
    <template #actions>
      <v-btn class="success" dark :to="localePath('/admin/camper')">
...
export default {
  ...
  methods: {
    submitForm () {
      ...
      this.$api.post('admin/addcamper', formData).then((res) => {
        // console.log(res.data)
        this.loading = false
        if (res.data.status) {
          this.message = res.data.message
          setTimeout(() => {
            this.$router.push({ path: '/admin/camper/' + res.data.data })
          }, 2000)
        } else {
  ...
  head () {
    return {
      title: this.$langAdmin('AddCamperTitle'),
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: this.$langAdmin('AddCamperDes')
        }
      ]
    }
  }
