    ...
    <v-simple-table>
      <template v-slot:default>
        <tbody>
          ...
          <WithdrawalList v-for="(item, index) in data" v-show="!loading" :key="index" :item="item" />
...
<script>
import WithdrawalList from '~/components/admin/lists/WithdrawalList'
export default {
  layout: 'admin',
  components: {
    WithdrawalList
  ...
  methods: {
    getData () {
      this.loading = true
      this.$api('admin/partner-withdrawal/' + this.$route.params.id + '/' + this.page).then((res) => {
        this.loading = false
  ...
  head () {
    return {
      title: this.$langAdmin('Partners'),
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: this.$langAdmin('Partners')
        }
      ]
    }
  }
