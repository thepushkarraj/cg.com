    ...
    <v-simple-table>
      <template v-slot:default>
        <tbody>
          ...
          <ContactList v-for="(item, index) in data" v-show="!loading" :key="index" :item="item" />
...
<script>
import ContactList from '~/components/admin/lists/ContactList'
export default {
  layout: 'admin',
  components: {
    ContactList
  ...
  methods: {
    getData () {
      this.loading = true
      this.$api('admin/contactlist/' + this.page).then((res) => {
        this.loading = false
