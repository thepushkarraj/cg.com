...
export default {
  ...
  methods: {
    getData () {
      this.loading = true
      this.$api('admin/cancel-bookinglist/' + this.page).then((res) => {
        this.loading = false
  ...
  head () {
    return {
      title: this.$langAdmin('CancelBookingsTitle'),
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: this.$langAdmin('CancelBookingsDes')
        }
      ]
    }
  }
