<template>
  <one-row v-if="!pageLoading" class="mb-5">
    ...
    <template #actions>
      <v-btn class="success" dark :to="localePath('/partner/camper/add')">
    ...
    <form-layout :error="error" :message="message" :loading="loading" :submit="$t('Submit')" @submit-form="submitForm">
      ...
      <v-row>
        <div class="col-md-12">
          <v-btn class="teal mr-3 mb-3" dark @click="ctab = 'General'">
            {{ $t('General') }}
          </v-btn>
          <v-btn class="teal mr-3 mb-3" dark @click="ctab = 'VehicleDetails'">
            {{ $t('VehicleDetails') }}
          </v-btn>
          ...
          <v-btn class="teal mr-3 mb-3" dark @click="ctab = 'SeasonalPrice'">
            {{ $t('SeasonalPrice') }}
          </v-btn>
        ...
        <div class="col-md-12">
          <h3 class="text-uppercase mb-2" v-text="$t(ctab)" />
          <v-divider />
        </div>
        <div class="col-md-12">
...
export default {
  ...
  methods: {
    getData () {
      ...
      this.getlAddonData()
      this.getlAddonCheck()
      this.$api('vendor/singlecamper/' + this.$route.params.id).then((res) => {
    ...
    submitForm () {
      ...
      this.$api.post('vendor/editcamper/' + this.$route.params.id, formData).then((res) => {
  ...
  head () {
    return {
      title: this.$langAdmin('EditCamper'),
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: this.$langAdmin('EditCamper')
        }
      ]
    }
  }
